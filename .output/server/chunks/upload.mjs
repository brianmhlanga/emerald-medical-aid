import { d as defineEventHandler, b as getQuery } from './nitro/node-server.mjs';
...
class PersistentFile extends EventEmitter {
  constructor({ filepath, newFilename, originalFilename, mimetype, hashAlgorithm }) {
    ...
    if (typeof this.hashAlgorithm === 'string') {
      this.hash = nodeCrypto.createHash(this.hashAlgorithm);
    } else {
      this.hash = null;
    }
  }
...
const OctetStreamParser$1 = OctetStreamParser;
...
async function init$2(_self, _opts) {
  ...
  this._parser = new OctetStreamParser$1(this.options);
...
const QuerystringParser$1 = QuerystringParser;
...
function init$1(_self, _opts) {
  ...
  const parser = new QuerystringParser$1(this.options);
...
function init(_self, _opts) {
  ...
  const parser = new JSONParser$1(this.options);
...
// src/index.ts
function readFiles(event, options) {
...
const upload = defineEventHandler(async (event) => {
  ...
  let imageName = String(performance.now()) + String(Math.round(Math.random() * 1e7));
  let newPath = `${path$1.join("uploads", imageName)}.png`;
  sharp(filepath).resize({
    width: 500,
    height: 500,
    fit: "contain",
    background: { r: 255, g: 255, b: 255, alpha: 0 }
  }).flatten({ background: "#fff" }).png().toFile(newPath);
  try {
