...
const sendIndividualEmail = async (result) => {
...
<form>
            ...
            <div class="mb-6">
                ...
                <div class="mb-6 dependency">
                    ...
                    ${result.depandancy.length > 0 ? result.depandancy.map((dependency, index) => `
                            <div>
                        ...
                        `).join("") : '<p class="text-gray-500">No dependencies found.</p>'}
                </div>
...
`;
  ...
  await transporter.sendMail({
    from: "<registrations@emeraldmas.com>",
    // sender address
    to: config.EMAIL_RECEIVER,
    // list of receivers
    subject: "NOTIFICATION OF NEW REGISTRATION (INDIVIDUAL)",
    // Subject line // plain text body
    html: htmlContent
    // html body
  });
};
const sendCivilEmail = async (result) => {
  const htmlContent = `<!DOCTYPE html>
<html lang="en">
...
`;
  ...
  await transporter.sendMail({
    ...
    to: config.EMAIL_RECEIVER,
    // list of receivers
    subject: "NOTIFICATION OF NEW REGISTRATION (CIVIL SERVANT)",
    // Subject line // plain text body
    html: htmlContent
    // html body
  });
...
export { sendIndividualEmail as a, sendCivilEmail as s };
